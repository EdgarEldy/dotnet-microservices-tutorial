...
/// <summary>
/// Reads order-api's Kafka topics from the beginning with a throwaway consumer group, the way
...
public static class KafkaTopicReader
{
    ...
    public static Task<KafkaReadResult> ReadUntilAsync(
        ...
        Task.Run(() =>
        {
            var config = new ConsumerConfig
            {
                ...
                GroupId = $"order-api-tests-{Guid.NewGuid():N}",
    ...
    /// <summary>True when the event body carries <paramref name="email"/> as its CustomerEmail field.</summary>
    public static bool HasEmail(string value, string email) =>
        string.Equals(GetString(value, "customerEmail"), email, StringComparison.OrdinalIgnoreCase);
