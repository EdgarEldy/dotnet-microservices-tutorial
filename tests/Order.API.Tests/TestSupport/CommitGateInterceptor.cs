...
/// <summary>
/// Test-only EF Core interceptor registered on order-api's AppDbContext. When armed, it waits
/// for the first transaction that, right before committing, contains an OutboxMessage whose body
/// mentions <see cref="CommitScenario.Marker"/> (a customer e-mail unique to one test). It then reads,
